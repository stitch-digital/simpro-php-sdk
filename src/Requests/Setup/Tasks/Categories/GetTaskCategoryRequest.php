<?php

declare(strict_types=1);

namespace Simpro\PhpSdk\Simpro\Requests\Setup\Tasks\Categories;

use Saloon\Enums\Method;
use Saloon\Http\Request;
use Saloon\Http\Response;
use Simpro\PhpSdk\Simpro\Data\Setup\TaskCategory;

final class GetTaskCategoryRequest extends Request
{
    protected Method $method = Method::GET;

    public function __construct(
        private readonly int|string $companyId,
        private readonly int|string $taskCategoryId,
    ) {}

    public function resolveEndpoint(): string
    {
        return "/api/v1.0/companies/{$this->companyId}/setup/tasks/categories/{$this->taskCategoryId}";
    }

    public function createDtoFromResponse(Response $response): TaskCategory
    {
        return TaskCategory::fromResponse($response);
    }
}
